...
t = SelectDate1.Extends(Calendar, "SelectDate1");
t.div_id="";
t.node=null;
t.parentDivNode;
t.parentInput_id="";
SelectDate1.prototype.render = function(id) {
	...
	div.innerHTML=this.getCalendarStr();
	t.parentDivNode=div;
};

SelectDate1.prototype.getCalendarStr = function() {
	...
	_th_leftarrow.attachEvent('onclick'function(){Instance(this.index).MiniCalendar_goPreMonth();} );
	_th_leftarrow.innerHTML="<<"; 
	_tr.appendChild(_th_leftarrow);
	
	var _th_rightarrow=document.createElement("th");
	_th_rightarrow.attachEvent('onclick'function(){Instance(this.index).MiniCalendar_goNextMonth();} );
	_th_rightarrow.attachEvent(>><<"; 
	...
	for ( var i = 1; i < _weekFirstDay; i++) {
		...
		_td_date.attachEvent('onclick',function(){Instance(this.index).selectAnotherMonDateTd(this)});
	...
	for ( var i = _weekLastDay + 1; i <= 7; i++) {
		...
		str += "<td id=\"calendar-mini-"+y+"-"+m+"-"+ (k)+"-"+this.index+"\" class=\"othermonth\" onclick=\"Instance('" + this.index
				+ "').selectAnotherMonDateTd(this)\">"+k+"</td>";k++;
	}
...
SelectDate1.prototype.selectAnotherMonDateTd = function(it) {
	var y =it.id.split("-")[2];
	var m =it.id.split("-")[3];
	var d =it.id.split("-")[4];
...
SelectDate1.prototype.MiniCalendar_goPreMonth = function(it) {
	this.goPreMonth();
			this.refreshView();

};

SelectDate1.prototype.MiniCalendar_goNextMonth = function(it) {
	this.goNextMonth();
			this.refreshView();

};
SelectDate1.prototype.MiniCalendar_select = function(e) {
...
SelectDate1.prototype.refreshView = function() {
while (this.parentDivNode.lastChild!=null)
  {
 	this.parentDivNode.removeChild(this.parentDivNode.lastChild);
  }
	this.parentDivNode.innerHTML = this.getCalendarStr();
	this.clear();

};
SelectDate1.prototype.selectDate = function(day) {
...
// 数据的初始化------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SelectDate1.prototype.clear = function() {
