...
var layer={
    ...
    if(!wait||wait.length==0){
     ...
     wait = document.createElement("div");
            wait.className ="fa fa-spinner wait";
...
var dialog={
   ...
   /* closeWindow:function(index){
    ...
    hideMask:function () {
        var mask=   document.getElementsByClassName("mask");//$(".mask");
    ...
    showMask:function(){
        var mask = document.getElementsByClassName("mask");//document.getElementsByClassName("mask");
...
/*	"<div class=\"zinfo-icon\"><i  class='"+jso.icon+"'></i></div>"+*///<img src=\""+jso.src+"\"/>
	"<div class=\"zinfo\" style=\"color:"+jso.fontcolor+"\">"+(jso.type== "prompt"?"<input id='zpromptText' type=\"text\" class=\"\" ></input>":
                                                              		jso.msg)+"</div></div>"+
	...
	(jso.type== "confirm"?"<button type=\"button\" class=\"sure btn btn-primary\" >确定</button><button type=\"button\" class=\"cancel btn btn-default\" >取消</button>":
		"<button class='sure btn btn-primary' style='width:95%'>确定</button>")+
	"</div></div>"+
	"</div>";
	var widgets= document.getElementsByClassName("widget");//document.getElementsByClassName("widget");
...
function zprompt(msg,title,okfn,cancelfn){
	zdialogue({"msg":msg,type:"prompt","title":msg,fontcolor:"#777777",src:PATH+"/static/img/exclamation.png",okfn:okfn,cancelfn:cancelfn,icon:"fa fa-info-circle"});
}
function zalert(msg,title,fn){
                    ...
                    for(var ruleName in cfg.rules[this.name]){console.log(ruleName);
                        if(that.methods[ruleName]){
                           var bool= that. methods[ruleName].call(that,getVal(this),this,cfg.rules[this.name][ruleName]);
                             var errorElement=getChild(form,"."+that.cfg["errorClass"]);// $(form).find("."+that.cfg["errorClass"]);
                           if(errorElement)//如果存在清理掉
...
var BaseValidator={
    ...
    while(wrap&& wrap!=document){
           if(wrap.className.indexOf("form-group" )>-1 ){
           break;
           }else{
...
function validate(input,ruleName,param){
    return _validator.methods[ruleName].call(that,getVal(input),input,param);
}
