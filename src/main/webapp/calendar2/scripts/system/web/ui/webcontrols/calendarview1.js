...
CalendarView.prototype.getCalendarScheduleStr = function() {
	...
	for (var i = 0; i < weekDays.length; i++) {
		...
		str += "<td  id=\"td_"
				+ weekDays[i].format("yyyy-MM-dd") + "\" " + css
				+ "   align=\"center\">";
...
CalendarView.prototype.loadEvents=function(){//alert("before loadevents");
	...
	AjaxFun("activity/getActivities",jso,this.showActivities.Apply(this));
};
...
function del(ceid){
	var ce = ca.getCalendarEvent(ceid);alert(ceid);
	synStack[ceid]=ce;
	//ce.changeFlag=true;
	ce.isdel=true;
	ce.lastChangeTime=(new Date()).getTime();
...
function loopCheckAndSave(){
	...
	if(arr.length>0){
		AjaxFun("activity/saveActivitys",{'jsonstr':	JSON.stringify(arr) },saveHandler);
	}else{
		setTimeout("loopCheckAndSave()",1000);
	}
