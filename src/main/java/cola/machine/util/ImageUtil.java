    ...
    public static String generatePoster(String smallImage) throws IOException {
        ...
        try {
            if(templateImage==null){
                File templatePoster = PathManager.getInstance().getWebRootPath().resolve("poster").resolve("src.jpg").toFile();
            ...
            String fileWithFixx = /*DateUtil.formatToString(new Date(), DateUtil.YYYYMMDDHHMMSS)
                    + RandomString.randomString(4) + ".jpg";*/
                    smallImage.replace(image.getQrcodeDir()+"/", "").replace(".gif", ".jpg");//.replace(".gif", ".jpg");
            ...
            // 上传文件到服务器
        /*  if(image.isServerEnable())
            ImageUtil.upload(new String[] { poster_dir + fileWithFixx });
*/
     ...
     * @throws IOException
     */
    /*public static String zipFileOnServer(String[] relatvieFilePaths) throws IOException {
        logger.debug("begin zipFileOnserver , the param is :" + relatvieFilePaths);
        ImageConfig image = Config.getInstance().getImage();
        ...
        try {
            ...
            String serverPath = image.getServerDir();
            if (!serverPath.endsWith("/")) {
                serverPath += "/";
            }
            String poster_zip_dir = image.getPosterZipDir().trim();
            if (poster_zip_dir.startsWith("/"))
                poster_zip_dir = poster_zip_dir.substring(1);
            if (!poster_zip_dir.endsWith("/"))
                poster_zip_dir += "/";

            String relativePath = poster_zip_dir + zipfile;
            if (relativePath.startsWith("/"))
                relativePath = relativePath.substring(1);

            String cmd = "rm -f " + serverPath + relativePath+" &&  mkdir -p " + serverPath + poster_zip_dir + " && zip -j " + serverPath + relativePath;
            for (int i = 0; i < relatvieFilePaths.length; i++) {
                cmd += " " + serverPath + relatvieFilePaths[i];
            }
            ...
            // sftpClient.mkdir(theDir, 6);
            session.close();

            con.close();
            return relativePath;
        } catch (IOException e) {
            logger.error("服务器给海报打zip包出错", e);
            throw e;
        }
...
*/
     ...
     * @throws IOException
     *//*
    public static String generateHaiBao(String[] smallImages, String bigImage, int width, int height)
            throws IOException {

        Path finalImages[] = new Path[smallImages.length];
        try {
            // 读入大图
            File big = new File(bigImage);
            BufferedImage bigBuffImage = ImageIO.read(big);
            for (int i = 0; i < smallImages.length; i++) {
                // 读入小图
                File small = new File(smallImages[i]);

                BufferedImage smallBuffImage = ImageIO.read(small);

                bigBuffImage.createGraphics().drawImage(smallBuffImage, width, height, null);
                String file = RandomGUIDUtil.generateKey();
                finalImages[i] = PathManager.getInstance().getPosterPath().resolve(file + ".jpg");
                ImageIO.write(bigBuffImage, "jpeg", finalImages[i].toFile());
            }

            byte[] buffer = new byte[1024];
            // 生成的ZIP文件名为Demo.zip
            Path strZipName = PathManager.getInstance().getPosterZipPath()
                    .resolve(RandomGUIDUtil.generateKey() + ".zip");
            ZipOutputStream out = new ZipOutputStream(new FileOutputStream(strZipName.toFile()));
            // 需要同时下载的两个文件result.txt ，source.txt
            for (int i = 0; i < finalImages.length; i++) {
                File file = finalImages[i].toFile();
...
*/
     ...
     * 上传文件到服务器
     * 
     * @param filePaths 是相对于webroot的路径
     */
    /*public static void upload(String[] filePaths) {
        ...
        try {
            ...
            for (int i = 0; i < filePaths.length; i++) {
                ...
                String localPath = PathManager.getInstance().getWebRootPath().resolve(filePath).toString();
                int index = filePath.lastIndexOf("/");
                String theDir = "";
                if (index != -1) {
                    ch.ethz.ssh2.Session session = con.openSession();
                    theDir = filePath.substring(0, index);
                    theDir = dir + "/" + theDir;
    ...
    public static boolean compressImg(String inputPath,String outputPath,Integer width,Integer height,boolean proportion){
        try{
