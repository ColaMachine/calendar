...
@Controller
@RequestMapping("/smsValidCode")
public class SmsValidCodeController extends BaseController{
    /** 日志 **/
    ...
    public String list() {
        return "/static/html/SmsValidCodeList.html";
    }
    ...
    public Object list(HttpServletRequest request) {
        ...
        List<SmsValidCode> smsValidCodes = smsValidCodeService.listByParams4Page(params);
        return ResultUtil.getResult(smsValidCodes, page);
    }
    ...
    public Object edit( HttpServletRequest request) {
        // 查找所有的角色
        return "/static/html/SmsValidCodeEdit.html";
    }
    @RequestMapping(value = "/view.htm")
    public Object viewPage( HttpServletRequest request) {
        return "/static/html/SmsValidCodeView.html";
    }
...
if(!StringUtil.isBlank(id)){
    SmsValidCode bean = smsValidCodeService.selectByPrimaryKey(Integer.valueOf(id));
...
return this.getResult(result);
    ...
    public Object save(HttpServletRequest request) throws Exception {
        ...
        String id = request.getParameter("id");
        if(!StringUtil.isBlank(id)){
            smsValidCode.setId(Integer.valueOf(id)) ;
        }
        
        String phone = request.getParameter("phone");
        if(!StringUtil.isBlank(phone)){
            smsValidCode.setPhone(String.valueOf(phone)) ;
        }
        
        String systemNo = request.getParameter("systemNo");
        if(!StringUtil.isBlank(systemNo)){
            smsValidCode.setSystemNo(String.valueOf(systemNo)) ;
        }
        
        String sendTime = request.getParameter("sendTime");
        if(!StringUtil.isBlank(sendTime)){
            smsValidCode.setSendTime(Timestamp.valueOf(sendTime)) ;
        }
        
        String status = request.getParameter("status");
        if(!StringUtil.isBlank(status)){
            smsValidCode.setStatus(Byte.valueOf(status)) ;
        }
        
        String reason = request.getParameter("reason");
        if(!StringUtil.isBlank(reason)){
            smsValidCode.setReason(String.valueOf(reason)) ;
        }
        */
        String id = request.getParameter("id");
        if(!StringUtil.isBlank(id)){
        ...
        if(!StringUtil.isBlank(sendTime)){
            if(StringUtil.checkNumeric(sendTime)){
                ...
                smsValidCode.setSendTime(new Timestamp( DateUtil.parseToDate(sendTime, "yyyy-MM-dd HH:mm:ss").getTime()));
            }
        }
        String status = request.getParameter("status");
        if(!StringUtil.isBlank(status)){
        ...
        return smsValidCodeService.save(smsValidCode);
    ...
    public Object multiDelete(HttpServletRequest request) {
       ...
       return  smsValidCodeService.multilDelete(idAry);
    }
    ...
    public Object exportExcel(HttpServletRequest request){
        ...
        // 查询list集合
        List<SmsValidCode> list =smsValidCodeService.listByParams(params);
        // 存放临时文件
