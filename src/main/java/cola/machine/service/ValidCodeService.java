...
public class ValidCodeService {
    ...
    public ResultDTO getSmsValidCode(String systemCode, String phone) {
      ...
      /*  ResultDTO result = validSystemNo(systemCode);
        ...
        if(systemConfig==null){
            return ResultUtil.getResult(1,2,, "系统代号不正确");
        }
        // 验证phone
        ...
        if (!StringUtil.isPhone(phone)) {
            return ResultUtil.getResult(303, "手机号格式错误");
        }
        // 取缓存中业务+手机号 的value
       // String mapValue = (String) CacheUtil.getInstance().readCache(systemCode + phone, String.class);
        ...
        if (history != null) {
           // String[] mapValueAry = mapValue.split("-");
            ...
            try {
                ...
                if (nowTime > (history.getLast()
                        + Integer.valueOf(systemConfig.getSmsLiveTime()))) {
                    // 超时
                    //应该把缓存中的清理掉
                ...
                if (nowTime < (history.getLast()
                        + Integer.valueOf(systemConfig.getSmsRefreshTime()))) {
                    // 刷新过快
                    return ResultUtil.getResult(304, "验证码刷新过快" );
                }
                if(!handleHistorys(history,systemConfig.getMaxRequestTime(),systemConfig.getRangeTime())){
                    return ResultUtil.getResult(305, "发送次数过多" );
                }
            } catch (Exception e) {
                log.error("validcode.live.time");
                return ResultUtil.getResult(306, "验证码生存周期配置有误");
            }
        }else{
        ...
        if ("fail".equals(rc)) {
            return ResultUtil.getResult(307, ResultUtil.fail, "短信发送失败");
        }
        ...
        if (result.isRight()) {
            ...
            try {
              ...
              //  CacheUtil.getInstance().writeCache(systemCode + phone, finalCode + "-" + nowTime,Integer.valueOf(systemConfig.getSmsLiveTime()));
            } catch (Exception e) {
                return ResultUtil.getResult(308, ResultUtil.fail, "短信发送失败");
    ...
    public ResultDTO validCode(String systemCode, String phone, String code,boolean isSms) {
        ...
        if (type==2&&!StringUtil.checkAlpha(code) )
        {
            return ResultUtil.getResult(302, "验证码错误");
        }
            ...
            if (mapValue != null) {
                ...
                try {
                    if (nowTime > (Long.valueOf(mapValueAry[1])
                            + Integer.valueOf(systemConfig.getImgLiveTime()))) {
                        // 超时
                        //应该把缓存中的清理掉
                    ...
                    if (nowTime < (Long.valueOf(mapValueAry[1])
                            + Integer.valueOf(systemConfig.getImgRefreshTime()))) {
                        // 刷新过快
        ...
        if (result.isRight()) {
            ...
            try {
                CacheUtil.getInstance().writeCache(systemCode + sessionid, finalCode + "-" + nowTime,
                        systemConfig.getImgLiveTime() / 1000);
            } catch (Exception e) {
                e.printStackTrace();
