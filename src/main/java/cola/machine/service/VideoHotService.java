...
@Service("videoHotService")
public class VideoHotService extends BaseService {
    private static final Logger logger = LoggerFactory
            .getLogger(VideoHotService.class);
    @Resource
    ...
    public ResultDTO save(VideoHot videoHot) {
        // 进行字段验证
       ValidateUtil<VideoHot> vu = new ValidateUtil<VideoHot>();
        ResultDTO result = vu.valid(videoHot);
        if (result.getR() != 1) {
            return result;
        }
         //逻辑业务判断判断
