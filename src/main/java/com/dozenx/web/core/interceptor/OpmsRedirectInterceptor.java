...
import com.dozenx.util.HttpPostUtil;
...
//    return false;
       /* Matcher slashMatcher = Pattern.compile("/").matcher(opmsSchema);
        ...
        HashMap<String, String> textParam = new HashMap<String, String>();
        ...
        while (paramNames.hasMoreElements()) {
             ...
             * "=" + paramValue); textParam.put(paramName, paramValue); } }
             */
            if(paramName.equals("url")){
                continue;
            }
            textParam.put(paramName, URLEncoder.encode(paramValue));
        }
        // 包括用户信息
     /*   textParam.put("usertoken", JsonUtil.toJson(userInfoMap));
        textParam.put("pagesize", ""+Integer.parseInt(SysConfigUtil.getParamValue("page.pageSize")));
       */
        ...
        if (request.getMethod().equals("POST") || request.getContentLength() > 297) {
            ...
            if(contentType.startsWith("application/json")) {// it may be like {a:1,b:2}
                ...
                for (Iterator<String> textParamIt = keySet.iterator(); textParamIt.hasNext(); ) {
                    String name = textParamIt.next();
                    String value = textParam.get(name);
